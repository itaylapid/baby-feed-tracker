...
<div class="overlay" id="settingsOverlay">
  <div class="sheet">
    ...
    <button class="text-btn" id="closeSettingsBtn" style="margin-top:16px;">סגור</button>
  </div>
</div>
...
// Local-storage backed replacement for the sandboxed `window.storage` API the
// original standalone HTML file was written against. Same get/set(key, value)
// shape, so the ported script below can be copied over almost verbatim.
const storage = {
  async get(key) {
...
function initApp() {
  const ICONS = {
    ...
    baby: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="9" r="5"/><path d="M9 9c0 1 .8 1.8 1.5 1M15 9c0 1-.8 1.8-1.5 1"/><path d="M12 14v2M7 21c0-3 2.2-5 5-5s5 2 5 5"/></svg>',
  ...
  async function loadData(){
    try{ const r=await storage.get(STORAGE_KEY,false); bottles=r&&r.value?JSON.parse(r.value):[]; }catch(e){ bottles=[]; }
    try{ const r=await storage.get(HISTORY_KEY,false); history=r&&r.value?JSON.parse(r.value):[]; }catch(e){ history=[]; }
    try{ const r=await storage.get(PROFILE_KEY,false); profile=r&&r.value?JSON.parse(r.value):{}; }catch(e){ profile={}; }
    try{ const r=await storage.get(LAST_FORMULA_KEY,false); lastFormulaAmount=r&&r.value?JSON.parse(r.value):130; }catch(e){ lastFormulaAmount=130; }
    try{ const r=await storage.get(NURSE_KEY,false); nursingActive=r&&r.value?JSON.parse(r.value):null; }catch(e){ nursingActive=null; }
    try{ const r=await storage.get(NIGHT_KEY,false); nightOverride=r&&r.value!==undefined?JSON.parse(r.value):null; }catch(e){ nightOverride=null; }
    try{ const r=await storage.get(WEIGHT_KEY,false); weightHistory=r&&r.value?JSON.parse(r.value):[]; }catch(e){ weightHistory=[]; }
    try{ const r=await storage.get(PUMP_ACTIVE_KEY,false); pumpingActive=r&&r.value?JSON.parse(r.value):null; }catch(e){ pumpingActive=null; }
    try{ const r=await storage.get(PHOTO_KEY,false); babyPhoto=r&&r.value?r.value:null; }catch(e){ babyPhoto=null; }
  ...
  async function saveBottles(){ try{ await storage.set(STORAGE_KEY,JSON.stringify(bottles),false); }catch(e){} }
  async function saveHistory(){ try{ await storage.set(HISTORY_KEY,JSON.stringify(history),false); }catch(e){} }
  async function saveProfile(){ try{ await storage.set(PROFILE_KEY,JSON.stringify(profile),false); }catch(e){} }
  async function saveLastFormula(){ try{ await storage.set(LAST_FORMULA_KEY,JSON.stringify(lastFormulaAmount),false); }catch(e){} }
  async function saveNursing(){ try{ await storage.set(NURSE_KEY, nursingActive?JSON.stringify(nursingActive):'', false); }catch(e){} }
  async function saveWeightHistory(){ try{ await storage.set(WEIGHT_KEY, JSON.stringify(weightHistory), false); }catch(e){} }
  async function savePumpActive(){ try{ await storage.set(PUMP_ACTIVE_KEY, pumpingActive?JSON.stringify(pumpingActive):'', false); }catch(e){} }
  async function savePhoto(){ try{ await storage.set(PHOTO_KEY, babyPhoto||'', false); }catch(e){} }
  ...
  $('nightToggle').addEventListener('click', async ()=>{
    ...
    applyNightMode();
    try{ await storage.set(NIGHT_KEY, JSON.stringify(nightOverride), false); }catch(e){}
  });
  ...
  function render(){
    const list=$('list');
    if(bottles.length===0){ list.innerHTML='<div class="empty">אין בקבוקים פעילים</div>'; }
    else{
  ...
  svg.addEventListener('mousedown', e=>{ dragging=true; handleMove(e.clientY); });
  window.addEventListener('mousemove', e=>{ if(dragging) handleMove(e.clientY); });
  window.addEventListener('mouseup', ()=>dragging=false);
  svg.addEventListener('touchstart', e=>{ dragging=true; handleMove(e.touches[0].clientY); e.preventDefault(); }, {passive:false});
  svg.addEventListener('touchmove', e=>{ if(dragging){ handleMove(e.touches[0].clientY); e.preventDefault(); } }, {passive:false});
  ...
  setInterval(()=>{ if(bottles.length) render(); renderBabyHeader(); }, 15000);
  loadData();
}
